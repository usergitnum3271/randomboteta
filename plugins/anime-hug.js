...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    let str;
    if (m.mentionedJid.length > 0) {
        str = `\`${name2}\` *عانقها بشدة* \`${name || who}\`.`;
    } else if (m.quoted) {
        str = `\`${name2}\` *أعطى عناق لـ* \`${name || who}\`.`;
    } else {
        str = `\`${name2}\` *عانق نفسه.*`.trim();
    }
...
handler.help = ['احضن @tag'];
handler.tags = ['anime'];
handler.command = ['hug','abrazar','عناق','يعانق','يحضن','حضن','احضن'];
handler.group = true;
