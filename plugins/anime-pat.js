...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    let str;
    if (m.mentionedJid.length > 0) {
        str = `\`${name2}\` *داعب* \`${name || who}\`.`; 
    } else if (m.quoted) {
        str = `\`${name2}\` *داعب* \`${name || who}\`.`;
    } else {
        str = `\`${name2}\` *قام بمداعبة نفسه.*`.trim();
    }
...
handler.help = ['ربت @tag'];
handler.tags = ['anime'];
handler.command = ['pat','acariciar','امسح-راسه','ربت','ربت-علي-راسه','طبطب','طبطب-علي-راسه','طبطبه'];
handler.group = true;
