...
const handler = async (m, { conn, args }) => {
  try {
    ...
    if (!chatId.endsWith('@g.us')) {
      return await conn.sendMessage(chatId, { text: `هذا الأمر يعمل فقط داخل المجموعات.`, quoted: m })
    }
    ...
    if (!messageToForward) {
      return await conn.sendMessage(chatId, { text: `يجب الرد على رسالة أو كتابة نص لعمل منشن للجميع.`, quoted: m })
    }
    ...
    console.error(error)
    await conn.sendMessage(m.chat, { text: `حدث خطأ أثناء تنفيذ أمر التاغ المخفي.`, quoted: m })
  }
}

handler.help = ['مخفي']
handler.tags = ['grupo']
handler.command = ['مخفي', 'hidden']
handler.group = true
handler.admin = true
